...
**Latest Posts（ツイート表示）**
- **API なし**: 表示するツイートIDを `X_TWEET_IDS`（カンマ区切り）で指定すると、X の oEmbed（認証不要）で埋め込み表示されます。未設定時はデフォルト3件を oEmbed 表示。
- **API あり**: `X_BEARER_TOKEN` を設定すると、最新3件を自動取得してカード風で表示します。
...
- **ランディング TOP ヒーロー**: `public/images/hero-profile.png`（ある場合こちらを優先表示）
